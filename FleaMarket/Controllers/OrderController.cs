...
namespace FleaMarket.Controllers
{
    public class OrderController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            //通过session的名字查询到该用户的ID
            int userId;
            string orderName = Session["LoginUser"].ToString();
            if (orderName == null)
            {
                orderName = Request.Cookies["isremember"].Value;
            }
            ShoppingEntities se = new ShoppingEntities();          
            Users user = se.Users.FirstOrDefault(p=>p.UserName == orderName);
            userId = user.UserID;

            //通过用户的ID查到产品
            var order = se.TradeRecord.Where(o => o.UserBuyID == userId).ToList();
            ViewBag.title = "我的订单";
            ViewBag.Id = userId;
            return View(order);
        }
